...
<div class="card mb-4">
  <div class="card-header d-flex flex-row align-items-center blue-gradient text-white">
    <a href="{{ route('users.show', ['name' => $meeting->user->name]) }}" class="text-dark m">
      <img src="{{ $meeting->user->profile_image }}" class="user-icon rounded-circle mr-3">
    </a>
    <a href="{{ route('users.show', ['name' => $meeting->user->name]) }}" class="text-white">
      <strong>{{ $meeting->user->name }}</strong> &nbsp;さんの練習日程表
      ...
      <div class="ml-auto card-text">
        <div class="dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-right">
            <a href="{{ route('meetings.edit', ['meeting' => $meeting]) }}" class="dropdown-item">
  ...
  <div class="card-body p-0">
    <table class="table table w-75 mx-auto mb-0">
      <tbody class="container">
        <tr class="row">
          <th class="col-4 border-top-0 font-weight-bold">タイトル</th>
          <td class="col-8 border-top-0">{{ $meeting->topic }}</td>
        </tr>
        ...
        @if (Auth::id() === $meeting->user_id)
          <tr class="row">
            ...
            <td class="col-8">
              <a href="{{ $meeting->start_url }}" class="text-primary">
                {{ substr($meeting->start_url, 0, 75) }} ...
              </a>
              <br>
              <small>(※ミーティングのホストだけに見えています)</small>
            </td>
          </tr>
        @endif
        <tr class="row">
          ...
          <td class="col-8">
            <a href="{{ $meeting->join_url }}" class="text-primary">
              {{ $meeting->join_url }}
            </a>
          </td>
        </tr>
